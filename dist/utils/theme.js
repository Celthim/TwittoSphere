...
export function save_theme() {
    localStorage.setItem("theme", get_current_theme());
}
export function load_theme(toggle_button) {
...
export function refresh_toggle_button(toggle_button, theme) {
    toggle_button.textContent = theme === "light" ? "🔆" : "🌙";
}
export function toggle_theme(toggle_button) {
    const new_theme = get_current_theme() === "light" ? "dark" : "light";
    set_theme(new_theme);
    save_theme();
